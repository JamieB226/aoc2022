...
function Get-TopCrate
{
    ...
    foreach ($MoveInstruction in ($MoveInstructions -split "`r`n"))
    {
        ...
        {            
            ...
            If($StartState[$From -1].count -ge $Count)
            {
                Write-Verbose "Movng $Count items from $From to $To.  $From currently has $($startstate[$from-1].count) items."
                $ToMove=$StartState[$From - 1].GetRange($StartState[$From-1].count-$Count,$count)
                $StartState[$To - 1].AddRange($ToMove)
                $StartState[$From - 1].RemoveRange($StartState[$From-1].count-$Count,$count)
            }
            else
            {
                Write-Debug "Not enough items to move"
            }
