...
function Get-SignalStrength
{
    ...
    for($i=1;$i -le 240;$i++)
    {
        $CurrentCycle=$Results | Where-Object {$_.CycleNumber -eq $i}
        $LitPixels=[System.Collections.ArrayList]::new()
        $LitPixels.Add(($i-1) - (40*$rowNumber)) | Out-Null
        $LitPixels.Add(($i) - (40*$rowNumber)) | Out-Null
        $LitPixels.Add(($i+1) - (40*$rowNumber)) | Out-Null
        if($CurrentCycle.FinalValue -in $LitPixels)
        {
        ...
        if(($CurrentCycle.CycleNumber % 40) -eq 0)
        {
